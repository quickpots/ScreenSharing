...
if (Test-Path -Path $recycleBinFolderPath) {
    try {
        ...
        Write-Host "RECYCLE BIN: " -NoNewline -ForegroundColor Cyan
...
$all | ForEach-Object {
    ...
    try {
        ...
        $cfg = (Get-CimInstance -ClassName Win32_Service -Filter "Name='$($_)'").StartMode
...
Write-Host "VPN Check:" -ForegroundColor Yellow
...
try {
    ...
    (Invoke-WebRequest "https://proxycheck.io/v2/$ip`?vpn=1&asn=1" -UseBasicParsing).Content |
        Set-Content $proxyPath

    Get-Content $proxyPath | Select-String "proxy"
}
catch {
